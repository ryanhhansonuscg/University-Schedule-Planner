...
test('index exposes keyboard-native navigation and accessible loading state', () => {
  const html = fs.readFileSync('index.html','utf8'); const js = fs.readFileSync('assets/app.js','utf8');
  assert.match(html, /<nav[^>]+aria-label=/); assert.match(html, /<a[^>]+href="planner.html"/); assert.match(html, /aria-busy="true"/);
  assert.match(html, /id="relationship-summary"/); assert.match(html, /id="explorer-status"[^>]+role="status"/);
  assert.doesNotMatch(html, /id="graph"[^>]+aria-live=/); assert.match(js, /button\.setAttribute\('aria-label'.*graphRelationship/);
});
test('planner exposes calendar switching and keyboard add controls', () => {
  ...
  assert.match(html,/id="import-schedule"[^>]+aria-describedby=/); assert.match(html,/id="planner-message"[^>]+tabindex="-1"/);
});
